...
projectContainer.addEventListener('change', async (e) => {
  ...
  if (!e.target.classList.contains("input-update-project-pic")) return;
  ...
  try {
    const res = await fetch(`/api/users/${currentUser._id}/projects/${projectId}/photo`, {
      method: "POST",
      body: formData
    });
    ...
    if (res.ok) {
      ...
      alert("❌ Failed to update image: " + updated.message);
    }
  } catch (err) {
...
if (editEmailBtn) {
  editEmailBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(`/api/users/${currentUser._id}/email`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        currentUser = updated;
        document.getElementById("email-id").textContent = currentUser.email;
        alert("✅ Email updated!");
      } else {
        alert("❌ Failed to update email: " + updated.message);
      }
    } catch (err) {
      alert("❌ Server error updating email.");
    }
  });
}
...
if (editPhoneBtn) {
  editPhoneBtn.addEventListener('click', async () => {
    ...
    if (!newPhone || newPhone === currentUser.phone) return;

    try {
      const res = await fetch(`/api/users/${currentUser._id}/phone`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        currentUser = updated;
        document.getElementById("phone-id").textContent = currentUser.phone;
        alert("✅ Phone updated!");
      } else {
        alert("❌ Failed to update phone: " + updated.message);
      }
    } catch (err) {
      alert("❌ Server error updating phone.");
    }
  });
}
...
if (editBioBtn) {
  editBioBtn.addEventListener("click", async () => {
    ...
    const currentBio = currentUser.about_desc || '';
    const newBio = prompt("Edit your bio:", currentBio);

    if (newBio === null || newBio === currentBio) return;

    try {
      const res = await fetch(`/api/users/${currentUser._id}/about`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        alert("❌ Failed to update bio: " + (updatedUser.message || "Unknown error"));
      }
    } catch (err) {
